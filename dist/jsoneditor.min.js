...
constructor:f,init:function(){var a=this;this.ready=!1;var b=this.options.theme||f.defaults.theme;a.options.selectable_fields&&(b+="_s");var c=f.defaults.themes[b];if(!c)throw"Unknown theme "+b;this.schema=this.options.schema,this.theme=new c,this.template=this.options.template,this.refs=this.options.refs||{},this.uuid=0,this.__data={};var d=f.defaults.iconlibs[this.options.iconlib||f.defaults.iconlib];d&&(this.iconlib=new d),this.root_container=this.theme.getContainer(),this.element.appendChild(this.root_container),this.translate=this.options.translate||f.defaults.translate,
...
window.requestAnimationFrame(function(){a.ready&&(a.validation_results=a.validator.validate(a.root.getValue()),a.root.showValidationErrors(a.validation_results),a.trigger("ready"),a.trigger("change"),a.options.selectable_fields&&window.jQuery("select.comparison").on("change",function(b){a.onChange()}))})})},getValue:function(){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before getting the value";return this.root.getValue()},setValue:function(a){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before setting the value";return this.root.setValue(a),this},setValues:function(a){var b=this;if(this.options.selectable_fields){var c=[];window.jQuery.each(a,function(a){c.push(a)}),c.sort(),window.jQuery.each(c,function(c,d){var e=b.getEditor(d),f=a[d];if(e){e.setValue(f.value);var g=window.jQuery('select.comparison[data-schemapath="'+d+'"]')[0];g&&(g.value=f.method)}})}else{this.varname=void 0;for(var d in a){this.varname=d;break}this.setValue(a[this.varname])}return this},getValues:function(){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before getting the value";var a=this,b={};return this.options.selectable_fields?window.jQuery("select.comparison:has(option[value!='dontcare']:selected)").each(function(c,d){b[d.dataset.schemapath]={method:d.options[d.selectedIndex].value};var e=a.getEditor(d.dataset.schemapath);e&&(b[d.dataset.schemapath].value=e.getValue())}):(b={},b[this.varname]=this.getValue()),b},highlightDiff:function(a){var b=this;a&&window.jQuery.each(a,function(a,c){var d=b.getEditor(c);if(d){var e=d.control?d.control:d.container;e.classList.add("has-error")}})},validate:function(a){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before validating";
// Custom value
// Custom value
 ...
 */
f.AbstractEditor=a.extend({onChildEditorChange:function(a){this.onChange(!0)},notify:function(){this.jsoneditor.notifyWatchers(this.path)},change:function(){this.parent?this.parent.onChildEditorChange(this):this.jsoneditor.onChange()},onChange:function(a){this.notify(),this.watch_listener&&this.watch_listener(),a&&this.change()},register:function(){this.jsoneditor.registerEditor(this),this.onChange()},unregister:function(){this.jsoneditor&&this.jsoneditor.unregisterEditor(this)},getNumColumns:function(){return 12},init:function(a){this.jsoneditor=a.jsoneditor,this.theme=this.jsoneditor.theme,this.template_engine=this.jsoneditor.template,this.iconlib=this.jsoneditor.iconlib,this.translate=this.jsoneditor.translate||f.defaults.translate,this.original_schema=a.schema,this.schema=this.jsoneditor.expandSchema(this.original_schema),this.options=c({},this.options||{},a.schema.options||{},a),a.path||this.schema.id||(this.schema.id="root"),this.path=a.path||"root",this.formname=a.formname||this.path.replace(/\.([^.]+)/g,"[$1]"),this.jsoneditor.options.form_name_root&&(this.formname=this.formname.replace(/^root\[/,this.jsoneditor.options.form_name_root+"[")),this.key=this.path.split(".").pop(),this.parent=a.parent,this.link_watchers=[],a.container&&this.setContainer(a.container)},setContainer:function(a){this.container=a,this.schema.id&&this.container.setAttribute("data-schemaid",this.schema.id),this.schema.type&&"string"==typeof this.schema.type&&this.container.setAttribute("data-schematype",this.schema.type),this.container.setAttribute("data-schemapath",this.path)},preBuild:function(){},build:function(){},postBuild:function(){var a=this;this.header&&this.header.getElementsByClassName("actual-field")[0]&&this.header.getElementsByClassName("actual-field")[0].addEventListener("click",function(b){a.jsoneditor.trigger("change")}),this.setupWatchListeners(),this.addLinks(),this.setValue(this.getDefault(),!0),this.updateHeaderText(),this.register(),this.onWatchedFieldChange()},setupWatchListeners:function(){var a=this;if(
// Watched fields
...
// Specific format
if(this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this,this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description)),this.format=this.schema.format,!this.format&&this.schema.media&&this.schema.media.type&&(this.format=this.schema.media.type.replace(/(^(application|text)\/(x-)?(script\.)?)|(-source$)/g,"")),!this.format&&this.options.default_format&&(this.format=this.options.default_format),this.options.format&&(this.format=this.options.format),this.format)
// Text Area
...
// TODO: table display format
throw"Not supported yet";this.header=document.createElement("span"),this.header.textContent=this.getTitle(),this.title=this.theme.getHeader(this,this.header),this.container.appendChild(this.title),this.container.style.position="relative",
// Edit JSON modal
...
// Control buttons
this.title_controls=this.theme.getHeaderButtonHolder(),this.title_controls.style.marginTop="-5px",this.editjson_controls=this.theme.getHeaderButtonHolder(),this.addproperty_controls=this.theme.getHeaderButtonHolder(),this.title.insertBefore(this.title_controls,this.title.childNodes[0]),this.title.appendChild(this.editjson_controls),this.title.appendChild(this.addproperty_controls),
// Show/Hide button
this.collapsed=!1,this.toggle_button=document.createElement("i"),this.toggle_button.className="glyphicon glyphicon-triangle-bottom",this.toggle_button.style.fontSize="14px",this.title_controls.appendChild(this.toggle_button),this.toggle_button.addEventListener("click",function(b){b.preventDefault(),b.stopPropagation(),a.collapsed?(a.editor_holder.style.display="",a.collapsed=!1,this.className="glyphicon glyphicon-triangle-bottom"):(a.editor_holder.style.display="none",a.collapsed=!0,this.className="glyphicon glyphicon-triangle-right")}),
...
var b=this.getElementEditor(0,!0);if(this.item_default=b.getDefault(),this.width=b.getNumColumns()+2,this.options.compact?(this.panel=document.createElement("div"),this.panel.style.overflow="auto",this.container.appendChild(this.panel)):(this.title=this.theme.getHeader(this,this.getTitle()),this.header=this.title,this.container.appendChild(this.title),this.title_controls=this.theme.getHeaderButtonHolder(),this.title_controls.style.marginTop="-5px",this.title.insertBefore(this.title_controls,this.title.childNodes[0]),this.schema.description&&(this.description=this.theme.getDescription(this.schema.description),this.container.appendChild(this.description)),this.panel=this.theme.getIndentedPanel(),this.panel.style.overflow="auto",this.container.appendChild(this.panel),this.error_holder=document.createElement("div"),this.panel.appendChild(this.error_holder)),this.panel.appendChild(this.table),this.controls=this.theme.getButtonHolder(),this.panel.appendChild(this.controls),this.item_has_child_editors)for(var c=b.getChildEditors(),d=b.property_order||Object.keys(c),e=0;e<d.length;e++){var f=a.theme.getTableHeaderCell(a,c[d[e]],c[d[e]].getTitle());c[d[e]].options.hidden&&(f.style.display="none"),a.header_row.appendChild(f)}else a.header_row.appendChild(a.theme.getTableHeaderCell(a,null,this.item_title));b.destroy(),this.row_holder.innerHTML="",
// Row Controls column
this.controls_header_cell=a.theme.getTableHeaderCell(a,null," "),a.header_row.appendChild(this.controls_header_cell),
// Add controls
...
f.defaults.editors["enum"]=f.AbstractEditor.extend({getNumColumns:function(){return 4},build:function(){this.container;this.title=this.header=this.label=this.theme.getFormInputLabel(this,this.getTitle()),this.container.appendChild(this.title),this.options.enum_titles=this.options.enum_titles||[],this["enum"]=this.schema["enum"],this.selected=0,this.select_options=[],this.html_values=[];for(var a=this,b=0;b<this["enum"].length;b++)this.select_options[b]=this.options.enum_titles[b]||"Value "+(b+1),this.html_values[b]=this.getHTML(this["enum"][b]);
// Switcher
...
if(this.title=this.header=this.label=this.theme.getFormInputLabel(this,this.getTitle()),
...
return this._super(a,b,c)},getFormInputField:function(a){var b=this._super(a);return"checkbox"!==a&&(b.className+="form-control"),b},getFormControl:function(a,b,c){var d=document.createElement("div");return a&&"checkbox"===b.type?(d.className+=" checkbox",a.appendChild(b),a.style.fontSize="14px",d.style.marginTop="0",d.appendChild(a),b.style.position="relative",b.style.cssFloat="left"):(d.className+=" form-group",a&&(a.className+=" control-label",d.appendChild(a)),d.appendChild(b)),c&&d.appendChild(c),d},getIndentedPanel:function(){var a=document.createElement("div");return a.className="well well-sm",a.style.paddingBottom=0,a},getFormInputDescription:function(a){var b=document.createElement("p");return b.className="help-block",b.innerHTML=a,b},getHeader:function(a,b){var c=document.createElement("h3");return c.style.whiteSpace="nowrap","string"==typeof b?c.textContent=b:c.appendChild(b),c},getHeaderButtonHolder:function(){var a=this.getButtonHolder();return a.style.marginRight="10px",a},getButtonHolder:function(){var a=document.createElement("div");return a.className="btn-group",a},getButton:function(a,b,c){var d=this._super(a,b,c);return d.className+="btn btn-default",d},getTable:function(){var a=document.createElement("table");return a.className="table table-bordered",a.style.width="auto",a.style.maxWidth="none",a},addInputError:function(a,b){a.controlgroup&&(a.controlgroup.className+=" has-error",a.errmsg?a.errmsg.style.display="":(a.errmsg=document.createElement("p"),a.errmsg.className="help-block errormsg",a.controlgroup.appendChild(a.errmsg)),a.errmsg.textContent=b)},removeInputError:function(a){a.errmsg&&(a.errmsg.style.display="none",a.controlgroup.className=a.controlgroup.className.replace(/\s?has-error/g,""))},getTabHolder:function(){var a=document.createElement("div");return a.innerHTML="<div class='tabs list-group col-md-2'></div><div class='col-md-10'></div>",a.className="rows",a},getTab:function(a){var b=document.createElement("a");return b.className="list-group-item",b.setAttribute("href","#"),b.appendChild(a),b},markTabActive:function(a){a.className+=" active"},markTabInactive:function(a){a.className=a.className.replace(/\s?active/g,"")},getProgressBar:function(){var a=0,b=100,c=0,d=document.createElement("div");d.className="progress";var e=document.createElement("div");return e.className="progress-bar",e.setAttribute("role","progressbar"),e.setAttribute("aria-valuenow",c),e.setAttribute("aria-valuemin",a),e.setAttribute("aria-valuenax",b),e.innerHTML=c+"%",d.appendChild(e),d},updateProgressBar:function(a,b){if(a){var c=a.firstChild,d=b+"%";c.setAttribute("aria-valuenow",b),c.style.width=d,c.innerHTML=d}},updateProgressBarUnknown:function(a){if(a){var b=a.firstChild;a.className="progress progress-striped active",b.removeAttribute("aria-valuenow"),b.style.width="100%",b.innerHTML=""}},getFormInputLabel:function(a,b){var c=document.createElement("label");c.appendChild(document.createTextNode(b));var d='<button class="btn btn-xs btn-add-notes pull-right" name="{text}" style="margin-left: 5px"> <i name="{text}" class="fa fa-sticky-note fa-lg"></i> </button>';d=d.replace("{text}",b),d=d.replace("{text}",b),c.innerHTML+=d;var e='<button class="btn btn-xs btn-add-enums pull-right" name="{text}" style="margin-left: 5px"> <i name="{text}" class="fa fa-pencil-square-o fa-lg"></i> </button>';return e=e.replace("{text}",b),e=e.replace("{text}",b),c.innerHTML+=e,c.className+=" clearfix",c.style.width="100%",c}}),f.defaults.themes.bootstrap3_s=f.defaults.themes.bootstrap3.extend({createSelect:function(a){var b=document.createElement("select");b.style.marginLeft="1em",b.style.border="1px solid #ccc",b.style.borderRadius="4px",b.style.backgroundColor="#fff",b.style.backgroundImage="none",b.dataset.schemapath=a.path,b.className="comparison";var c=[{id:"dontcare",name:"don't care",selected:!0},// don't compare
{id:"equals",name:"=="},// use strict equals
{id:"notequals",name:"!="},{id:"empty",name:"empty"},{id:"notempty",name:"doesn't empty"}];// use regex for compare
// use regex for compare
// order doesn't matter for comarison
return a.options.schema.$ref?null:("string"===a.options.schema.type?(c.push({id:"regex",name:"regex"}),c.push({id:"contains",name:"contains"}),c.push({id:"notcontains",name:"doesn't contain"})):"number"===a.options.schema.type?(c.push({id:"greater",name:">"}),c.push({id:"egreater",name:">="}),c.push({id:"less",name:"<"}),c.push({id:"eless",name:"<="})):"array"===a.options.schema.type&&(c.push({id:"weakequals",name:"weak equals (without order)"}),c.push({id:"actualinwo",name:"all actual in expected w/o ordering"}),c.push({id:"actualino",name:"all actual in expected with ordering"}),c.push({id:"expectedinwo",name:"all expected in actual w/o ordering"}),c.push({id:"expectedino",name:"all expected in actual with ordering"})),window.jQuery.each(c,function(a,c){var d=document.createElement("option");d.value=c.id,d.innerHTML=c.name,c.selected&&(d.selected="selected"),b.appendChild(d)}),b)},getTableHeaderCell:function(a,b,c){var d=document.createElement("th");if(d.style=d.style||{},d.appendChild(document.createTextNode(c))," "!=c&&(d.style.minWidth="200px",d.style.verticalAlign="top",b)){var e=this.createSelect(b);e&&(e.style.display="block",e.style.marginLeft="",d.appendChild(e))}return d},getHeader:function(a,b){var c=document.createElement("h3");c.style.whiteSpace="nowrap",c.style.display="inline-block","string"==typeof b?c.textContent=b:c.appendChild(b);var d=this.createSelect(a);if(d){var e=document.createElement("span");return e.appendChild(c),e.appendChild(this.createSelect(a)),e}return c},getFormInputLabel:function(a,b){var c=document.createElement("label");c.appendChild(document.createTextNode(b)),c.appendChild(this.createSelect(a));var d='<button class="btn btn-xs btn-add-notes pull-right" name="{text}" style="margin-left: 5px"> <i name="{text}" class="fa fa-sticky-note fa-lg"></i> </button>';d=d.replace("{text}",b),d=d.replace("{text}",b),c.innerHTML+=d;var e='<button class="btn btn-xs btn-add-enums pull-right" name="{text}" style="margin-left: 5px"> <i name="{text}" class="fa fa-pencil-square-o fa-lg"></i> </button>';return e=e.replace("{text}",b),e=e.replace("{text}",b),c.innerHTML+=e,c.className+=" clearfix",c.style.width="100%",c}}),f.AbstractIconLib=a.extend({mapping:{collapse:"",expand:"","delete":"",edit:"",add:"",cancel:"",save:"",moveup:"",movedown:""},icon_prefix:"",getIconClass:function(a){return this.mapping[a]?this.icon_prefix+this.mapping[a]:null},getIcon:function(a){var b=this.getIconClass(a);if(!b)return null;var c=document.createElement("i");return c.className=b,c}}),f.defaults.iconlibs.bootstrap3=f.AbstractIconLib.extend({mapping:{collapse:"chevron-down",expand:"chevron-right","delete":"remove",edit:"pencil",add:"plus",cancel:"floppy-remove",save:"floppy-saved",moveup:"arrow-up",movedown:"arrow-down"},icon_prefix:"glyphicon glyphicon-"}),f.defaults.templates["default"]=function(){return{compile:function(a){var b=a.match(/{{\s*([a-zA-Z0-9\-_ \.]+)\s*}}/g),c=b&&b.length;
