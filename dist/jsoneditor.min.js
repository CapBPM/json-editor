...
window.requestAnimationFrame(function(){a.ready&&(a.validation_results=a.validator.validate(a.root.getValue()),a.root.showValidationErrors(a.validation_results),a.trigger("ready"),a.trigger("change"))})})},getValue:function(){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before getting the value";return this.root.getValue()},setValue:function(a){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before setting the value";return this.root.setValue(a),this},setValues:function(a,b){if(void 0===b){b="root.";for(var c in a)return this.varname=c,void this.setValues(a[this.varname],b)}var d;if("object"!=typeof a&&"undefined"!=typeof a)d=this.getEditor("root"),d.setValue(a),$(this.element).find("div[data-schemapath='root'] .actual-field").attr("checked","checked");else for(var e in a){var f=b+e,g=a[e];!(g instanceof Array)&&g instanceof Object||(d=this.getEditor(f),d&&(d.setValue(a[e]),$(this.element).find("div[data-schemapath='"+f+"'] .actual-field").attr("checked","checked"))),g instanceof Object&&this.setValues(g,f+".")}return this},getValues:function(){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before setting the value";var a={},b=function(a,b){var c={},d=c;if(a.length>1){for(var e=1;e<a.length-1;++e)void 0===c[a[e]]&&(c[a[e]]={}),c=c[a[e]];c[a[a.length-1]]=b}else c=b;return d};if(this.options.selectable_fields){var c=this;$(this.element).find("input.actual-field:checked").parent().parent().parent().each(function(d,e){var f=$(e).attr("data-schemapath"),g=c.getEditor(f);g&&("root"===f?a=g.getValue():$.extend(!0,a,b(f.split("."),g.getValue())))})}else a=this.getValue();var d={};return d[this.varname]=a,d},highlightDiff:function(a){if(a){$("div[data-schemapath]").attr("style","color:initial");for(var b in a){var c=a[b];$("div[data-schemapath='"+c.replace(c.split(".")[0],"root")+"'] h3")[0]?$("div[data-schemapath='"+c.replace(c.split(".")[0],"root")+"'] h3").attr("style","color:red"):$("div[data-schemapath='"+c.replace(c.split(".")[0],"root")+"']").attr("style","color:red"),$("td[data-schemapath='"+c.replace(c.split(".")[0],"root").replace(/\[/g,".").replace(/\]/g,"")+"'] div ").addClass("has-error"),$("td[data-schemapath='"+c.replace(c.split(".")[0],"root").replace(/\[/g,".").replace(/\]/g,"")+"'] div .form-control").css("border-color","red")}}},validate:function(a){if(!this.ready)throw"JSON Editor not ready yet.  Listen for 'ready' event before validating";
// Custom value
// Custom value
